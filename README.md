# Đánh giá tiềm năng NST — Team US

Ứng dụng chấm điểm ứng viên/NST theo bộ tiêu chí nội bộ của **Team US**
(mỗi câu 1–5 điểm), tự động phân loại kết quả và lưu vào kho dữ liệu.

Đây là bản tách riêng chỉ dành cho Team US (không còn bước chọn team).
...
2. Bấm **New repository**, đặt tên (vd `nst-scoring-us`), chọn **Public**
...
1. Tạo Dataset repo trên Hugging Face: vào https://huggingface.co/new-dataset,
   đặt tên vd `nst-scoring-results-us` (đuôi `-us` để không trùng/lẫn với
   dữ liệu của Team VN), chọn **Private**, **Create dataset**.
   Ghi nhớ đường dẫn `<ten-user>/nst-scoring-results-us`.
...
4. Dán nội dung sau vào ô Secrets (thay giá trị thật của bạn), rồi **Save**:
   ...
   HF_DATASET_REPO = "ten-user/nst-scoring-results-us"
   ```
...
`https://huggingface.co/datasets/<ten-user>/nst-scoring-results-us`, kể cả khi
...
5. Vào **Settings → Variables and secrets** của Space, thêm:
   ...
   - Variable `HF_DATASET_REPO` = `<ten-user>/nst-scoring-results-us` (tạo
